...
const OrderHistory = () => {
  ...
  // 📊 Aggregate quantities by product for bar & pie charts
  const productTotals = orders.reduce((acc, { product, quantity, total }) => {
    if (!acc[product]) {
      acc[product] = { quantity: 0, total: 0 };
    }
    acc[product].quantity += quantity;
    acc[product].total += total;
    return acc;
  }, {});
  ...
  // 1️⃣ Bar Chart - Quantity per Product
  const barData = {
    labels: products,
    datasets: [
      {
  ...
  // 2️⃣ Pie Chart - Spending by Product
  const pieData = {
    labels: products,
    datasets: [
      {
  ...
  const lineData = {
    labels: sortedOrders.map(o => o.date),
    datasets: [
      {
        label: "Quantity Purchased Over Time",
        data: sortedOrders.map(o => o.quantity),
        fill: false,
        borderColor: "#17a2b8",
